...
plt.ylim(0, 1.05)
plt.grid(linestyle="--")
plt.title("ROC curve pour la distance euclidienne minim faites sur l'accélération normalisée \n AUPCR : {}".format(round(area(precisions,recalls),5)))
plt.show()
...
plt.title("Evolution du F1 score en fonction du seuil - Accélération normalisée \n F1 max pour seuil = {}".format(round(best_f1_thresh,2)))
plt.xlabel("Seuil")
plt.ylabel("f1 score")
plt.show()
