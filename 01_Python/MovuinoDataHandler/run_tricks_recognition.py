...
############   SETTINGS   #############
completeSequencesPath = "..\\..\\06_Data\\Raw_sequences\\sesh_151121_\\record_9_interpolated.csv"
...
events_interval = detection_model.detect(completeSequence, size_window,overlap,prominence,distance)

# time_win, tricks_interval_temp, peaks_gyr, peaks_a, peaks_tricks = sa.event_detection(completeSequence, size_window,overlap,prominence,distance)
# events_interval = sa.center_events(completeSequence,  tricks_interval_temp)

#------------

seuil = 0.0121

gyrNormalize_360Flip = array_gyr_normalize(ref_360flip.rawData)
gyrNormalize_ollie = array_gyr_normalize(ref_ollie.rawData)
gyrNormalize_kickflip = array_gyr_normalize(ref_kickflip.rawData)
gyrNormalize_heelflip = array_gyr_normalize(ref_heelflip.rawData)
gyrNormalize_pop_shovit = array_gyr_normalize(ref_pop_shovit.rawData)
gyrNormalize_fs_shovit = array_gyr_normalize(ref_fs_shovit.rawData)
...
for i, interval in enumerate(events_interval):
    ...
    newDF = correction_interpolation(tricks)
    newDF = sk.SkateboardXXX3000DataSet.normalized_L2(newDF.copy())

    #distance euclidienne
    tricksGyr_normalized = array_gyr_normalize(newDF)
