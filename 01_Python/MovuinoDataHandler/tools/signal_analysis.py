...
def event_detection(skateDataSet,size_window,overlap, prominence, distance):
    if size_window % 2 == 0:
        size_window += 1
...
def center_events(skateDataSet,tricks_interval, dt_f = 0.6):
    time_list = []
    Te = skateDataSet.Te
    for k in range(len(tricks_interval)):
        ...
        index_mean_loc = mean_time(normGyroscope)
